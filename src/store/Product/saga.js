...
function* callApiList({ params }) {
  try {
    const response = yield call(get, ENDPOINT.LIST_PRODUCT, params);
...
function* callApiDelete({ id }) {
  try {
    const response = yield call(puts, ENDPOINT.ACTIVE_PRODUCT + id);
    if (response.status === 200) {
...
function* callApiDetail({ id }) {
  try {
    const response = yield call(get, ENDPOINT.DETAIL_PRODUCT + id);
    if (response.status === 200) {
...
function* callApơUpdateDetail({ params }) {
  try {
    ...
    const response = yield call(
      puts,
      ENDPOINT.UPDATE_DETAIL_PRODUCT + id,
      params
    );
