...
function* callApiList({ params }) {
  try {
    const response = yield call(get, ENDPOINT.LIST_FACTORY, params);
...
function* callApiAdd({ params }) {
  try {
    const response = yield call(post, ENDPOINT.ADD_FACTORY, params);
...
function* callApiEdit({ params }) {
  try {
    ...
    const response = yield call(puts, ENDPOINT.EDIT_FACTORY + id, {
      name,
    });
...
function* callApiDelete({ id }) {
  try {
    const response = yield call(remove, ENDPOINT.DELETE_FACTORY + id);
    if (response.status === 200) {
...
export default function* factorySaga() {
  yield all([
